...
serve(async (req) => {
  ...
  try {
    ...
    if (!workout_name || !workout_content) {
      return new Response(JSON.stringify({ error: 'Nome e conteúdo do treino são obrigatórios.' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
